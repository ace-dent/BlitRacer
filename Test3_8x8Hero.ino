...
void test8x8Hero(bool byteAligned) {
    ...
    int16_t  x, y;              // Image position for rendering loops
    uint32_t loopCount = 0;     // Keep large integer, used for CPU baseline
    ...
    for (i = 0; i < 3; i = i + 1) {
        ...
        for (y = -8; y < HEIGHT; y = y + yStep) {
            for (x = -8; x < WIDTH; x = x + xStep) {
